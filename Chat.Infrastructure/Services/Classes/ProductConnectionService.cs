...
namespace Chat.Infrastructure.Services.Classes;
...
public sealed class ProductConnectionService : IProductConnectionService
{
    ...
    public async Task<string> GetResolvedConnectionStringAsync(CancellationToken cancellationToken = default)
    {
        ...
        var localFallback = GetLocalChatConnectionString();
        ...
        if (!useApi)
        {
            ...
            throw new InvalidOperationException(
                "SoftOnCloud:UseProductConnectionApi is false and neither ConnectionStrings:DefaultConnection nor MasterConnection is configured.");
        }
        ...
        if (productId is null or <= 0)
        {
            if (!string.IsNullOrWhiteSpace(localFallback))
            {
                _logger.LogWarning("SoftOnCloud:ProductId is not configured; using local chat connection fallback.");
                return localFallback;
            }
        ...
        if (string.IsNullOrWhiteSpace(bearerToken))
        {
            if (!string.IsNullOrWhiteSpace(localFallback))
            {
                _logger.LogWarning("No JWT on the request; using local chat connection fallback.");
                return localFallback;
            }
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            {
                _logger.LogWarning("Falling back to local chat connection after Product Connection API failure.");
                return localFallback;
            }
    ...
    /// <summary>
    /// Local Chat DB: DefaultConnection if set, otherwise MasterConnection (Kenil SOC_SaaS_Product).
    /// </summary>
    private string? GetLocalChatConnectionString()
    {
        ...
        {
            _logger.LogInformation("Using MasterConnection for chat DB (DefaultConnection not configured).");
